...
export class HeaderProcessor {
     ...
     */
    public static readHeader(event: APIGatewayProxyExtendedEvent, name: string): string | null {
     ...
     */
    public static readMultiValueHeader(event: APIGatewayProxyExtendedEvent, name: string): string[] {
     ...
     */
    public static readCookieValue(event: APIGatewayProxyExtendedEvent, name: string): string | null {

        let result = '';
        const headers = HeaderProcessor.readMultiValueHeader(event, 'cookie');
        headers.forEach((h) => {

            const data = parseCookie(h);
            if (data[name]) {
                result = data[name];
            }
        });
