...
LOGIN_COOKIES_FILE='login_cookies.txt'
MAIN_COOKIES_FILE='main_cookies.txt'
WEB_BASE_URL='https://web.authsamples-dev.com'
OAUTH_AGENT_BASE_URL='https://bfflocal.authsamples-dev.com:444/oauth-agent'
API_BASE_URL='https://bfflocal.authsamples-dev.com:444/investments'
...
#
# 2. Next verify that an OPTIONS request with no origin header does not return CORS headers
#
echo '2. OPTIONS request for an untrusted origin ...'
HTTP_STATUS=$(curl -i -s -X OPTIONS "$OAUTH_AGENT_BASE_URL/login/start" \
-H 'origin: https://www.untrusted.com' \
-o $RESPONSE_FILE -w '%{http_code}')
if [ "$HTTP_STATUS" != '204' ]; then
...
-H "access-control-request-headers: token-handler-version" \
-o $RESPONSE_FILE -w '%{http_code}')
echo "$HTTP_STATUS"
if [ "$HTTP_STATUS" != '204'  ]; then
...
if [ "$ALLOWED_HEADERS" != 'token-handler-version' ]; then
...
#
# 14. Next try to refresh the token and we should get a session_expired error
#
