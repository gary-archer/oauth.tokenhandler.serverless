...
######################################################
# A script to set up the authorizer with valid cookies
######################################################
...
#
# Update the cookie authorizer with valid secure cookies
#
echo "$(cat data/cookieAuthorizer.json | jq --arg i "$ANTI_FORGERY_TOKEN" '.headers."x-mycompany-csrf" = $i')"  > data/cookieAuthorizer.json
echo "$(cat data/cookieAuthorizer.json | jq --arg i "$AT_COOKIE_TEXT"     '.multiValueHeaders.cookie[0] = $i')" > data/cookieAuthorizer.json
echo "$(cat data/cookieAuthorizer.json | jq --arg i "$CSRF_COOKIE_TEXT"   '.multiValueHeaders.cookie[1] = $i')" > data/cookieAuthorizer.json
